...
export async function GET(request:NextRequest,{params}:{params:{id:string}}){
  ...
  try{
    const res = await fetch(`${baseUrl}/teams/${id}/matches`,{
      headers:{
        'X-Auth-Token':token
      },
      next:{
    ...
    if(error instanceof Error){
      console.error("Error fetching matches of specific team:",error.message, error.stack);
    }else{
      console.error("Unknown error fetching matches of specific team:",error)
    }
